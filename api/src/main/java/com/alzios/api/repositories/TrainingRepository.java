...
public interface TrainingRepository extends JpaRepository<Training, Long> {
     ...
     */
    @Query(value = " SELECT t.* FROM training t, program p, availability a, user u, program_availabilities pa, user_availabilities ua WHERE t.program_id = p.id AND pa.program_id = p.id AND pa.availabilities_id = a.id AND ua.user_id = u.id AND ua.availability_id = a.id AND p.goal_id = u.goal_id AND a.layout = t.layout AND u.id = ?1",nativeQuery = true)
